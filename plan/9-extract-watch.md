...
**Forced-K vs in-progress git operations (resilience-flagged interaction):** forced extraction exists to bound staleness of a tree someone is actually editing; a mid-rebase or mid-merge tree is transient by construction, and publishing it deliberately would reintroduce exactly what the stability rule avoids. The **forced**-K path is therefore deferred while the repo's `.git` carries an operation marker (`rebase-merge/`, `rebase-apply/`, `MERGE_HEAD`, `CHERRY_PICK_HEAD`, `BISECT_LOG` — one lstat each, no extra spawn). The normal stability path still runs, so an interactive rebase paused at a stop — a real, inspectable state — extracts normally once stable, and converges again at the next stop (accepted; the resilience report registers the banner-per-stop fatigue on the reader side). A marker left behind by an abandoned operation defers forced-K until the marker goes; bounded staleness resumes as soon as git's own state says the tree is whole. Deliberate tie-break, documented.

### Startup pass (A1-P2-1)

On `--watch` startup, after config validation and confinement checks, **every target is extracted once, serially, in entry order — before the first tick**. No state persists between watcher runs, so this is the only way edits made while the watcher was off ever reach `out`; skip-identical makes the no-change case free (no write, no downstream reload). Measured cost: ~**6.3 s** per corpus-sized repo (resilience bench), so N targets cost ~N×6.3 s of serial startup, each printing its summary block. Each repo's baseline fingerprint is the one captured at its startup extraction.
...
7. **change → re-extract → atomic rewrite**: at startup, both targets are extracted once before the first tick, and a target whose `out` already matches gets no write (A1-P2-1); then: edit a tracked file in repo A → A's `out` is rewritten with the new content; repo B's `out` mtime is untouched (multi-repo re-extracts only the changed repo).
