...
// detail panel renders `Evidence`, `GuideEdge` and `InternalBucket` values directly,
...
const CODEBASEGUIDE_ROOT = fileURLToPath(new URL('../..', import.meta.url));
const SRC = join(CODEBASEGUIDE_ROOT, 'src');
...
describe('CodebaseGuide adds files; it modifies none (§14)', () => {
  it('no script reads or writes web/, crates/, schema/, graph-analytics/ or fixtures/', () => {
    const pkg = JSON.parse(
      readFileSync(join(CODEBASEGUIDE_ROOT, 'package.json'), 'utf8'),
    ) as { scripts?: Record<string, string> };
    const scripts = Object.values(pkg.scripts ?? {}).join(' ');
    for (const forbidden of ['web/', 'crates/', 'schema/', 'graph-analytics/', 'fixtures/']) {
      // `tools/extractor/fixtures/` is CodebaseGuide's OWN fixtures directory; the
