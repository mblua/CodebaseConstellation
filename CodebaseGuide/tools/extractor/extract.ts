...
export const GENERATOR_NAME = 'codebaseguide-extract';
...
export function extract(options: ExtractOptions): ExtractResult {
  ...
  const nodes: GuideNode[] = [...ownership.nodes, ...apps.nodes];
  const edges: GuideEdge[] = [...apps.edges, ...ts.edges, ...rust.edges, ...commands.edges];
