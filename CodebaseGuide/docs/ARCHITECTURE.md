# CodebaseGuide — Architecture

CodebaseGuide is a local, self-contained web app that renders a repository as a **blueprint you can read without reading code**: repositories, applications, packages/crates, directories and files as nodes; typed relations between them; a hierarchy you expand and collapse by double-click.

It lives at `CodebaseGuide/` inside the CodebaseConstellation repository. It does not depend on the existing `web/` + SQLite + WebGL + `graph-analytics` pipeline, which continues to work unchanged. §14 states exactly which principles of that pipeline are preserved and which implementation is deliberately not reused.
...
* `adapters/` import `ports/` (plus their own library).
* **`src/main.tsx` is the composition root — the only module that imports a concrete adapter and a UI root, and wires them together.**
...
These are not conventions. An architecture test (§12) parses every source file with the TypeScript API and fails the build on any violating import — in particular, on any import of a renderer library from `contract/`, `domain/`, `projection/`, `app/` or `ui/`.
...
One versioned JSON document is the only thing that crosses the app boundary. The extractor writes it; the app reads it, and writes it back.
...
```jsonc
{
  "formatVersion": "1.0",
  "generator": {
    "name": "codebaseguide-extract", "version": "0.1.0",
...
`GuideNode.parentId` gives each entity exactly one parent. But membership is N:M (§5.3). So an app-centric hierarchy — in which a *file* or a *package* appears under **every application that bundles it** — cannot be expressed by re-parenting, without either duplicating the entity or arbitrarily picking one owner and losing the rest. A new `kind` string does nothing about that.
...
`CodebaseGuide/tools/extractor/`. A Node/TypeScript CLI inside CodebaseGuide's own package, sharing `src/contract/` with the app.
...
Node rather than Rust because it gives **one toolchain and a shared contract**, and because the TypeScript compiler API is needed anyway (§10.2). A Rust crate under `CodebaseGuide/` is **possible** — a nested or excluded workspace would keep it out of the root `members = ["crates/*"]` — just costlier for this product. The earlier "Cargo makes it impossible" was a false dichotomy.
...
| **architecture** | The import DAG (§2), by parsing every source with the TypeScript API: no renderer library outside `adapters/`; `app/` never imports `ui/`; nothing imports outside `CodebaseGuide/`. Plus the **AST sink check** (§11). |
...
**CodebaseGuide adds files. It modifies none.** Its own `package.json`, lockfile, `.gitignore` and `node_modules`; it imports nothing from `web/`, `crates/`, `schema/`, `graph-analytics/` or `fixtures/`; it is **not** added to the root `Cargo.toml`, which stays `members = ["crates/*"]`. The architecture test asserts no import escapes `CodebaseGuide/`, and the boundary test also asserts that CodebaseGuide's **scripts** neither read nor write `web/` artifacts.

### What is preserved

These principles come from `schema/README.md`, ADR 0001 and `constellation-ingest`, and they were right. CodebaseGuide keeps them — in its own code:
...
§2 says "`ui/` imports `app/` and `ports/`". The detail panel renders `Evidence`, `GuideEdge`
