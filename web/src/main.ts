...
function statusBadge(status: CapabilityInfo["status"], label = status): HTMLSpanElement {
...
class ConstellationApp {
  ...
  #renderDatasetUI(): void {
    ...
    this.#snapshotMeta.textContent = `${revision} · ${dataset.snapshot.historyMode} history · ${dataset.layout.algorithm}`;
  ...
  #renderLayers(): void {
    ...
    for (const layer of LAYER_DEFINITIONS) {
      ...
      if (layer.key === "change" && changeCapability.status !== "available") {
  ...
  #allCapabilities(): CapabilityInfo[] {
    ...
    return [...existing.values()].sort((left, right) => left.capability.localeCompare(right.capability));
  }
  ...
  #renderGlobalOverview(): void {
    ...
    header.append(
      createElement("p", "eyebrow", "START WITH STRUCTURE"),
      createElement("h2", undefined, "Architecture overview"),
      createElement(
        "p",
        undefined,
  ...
  #renderNodeDetail(detail: NodeDetail): void {
    ...
    if (detail.metrics.length === 0) {
      ...
      for (const metric of detail.metrics) {
        ...
        card.title = metric.provenance;
        card.append(
          createElement("strong", undefined, formatMetricValue(metric.value, metric.unit)),
          createElement("span", undefined, formatKind(metric.key)),
        );
    ...
    if (detail.findings.length === 0) {
      ...
      const stack = createElement("div", "finding-stack");
      for (const finding of detail.findings) stack.append(this.#findingCard(finding));
      findingsSection.append(stack);
    }
    content.append(findingsSection);
  ...
  #findingCard(finding: FindingInfo): HTMLElement {
    const card = createElement("article", `finding-card severity-${finding.severity}`);
