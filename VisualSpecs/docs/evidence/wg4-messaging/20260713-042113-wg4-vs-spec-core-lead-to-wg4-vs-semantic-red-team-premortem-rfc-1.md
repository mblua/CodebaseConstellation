# Independent semantic premortem: issue #1
...
Review the same full plan independently at `plan/1-enforce-main-pr-branch-names.md`. The current commit is a draft only; no ruleset or PR exists yet. Do not consult or adopt the resilience review.

Actively try to falsify naming semantics, issue identity/state checks, repository targeting, cutoff/grandfather meaning, exemptions, strict-current-main behavior, and the claim that the ruleset enforces the requested policy. Include the smallest reproducible counterexample, violated invariant/criterion, evidence, impact, and P0/P1/P2 severity for each finding.
