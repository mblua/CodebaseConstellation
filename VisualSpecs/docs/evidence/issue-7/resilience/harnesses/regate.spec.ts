...
for (const access of ['editable', 'readonly'] as const) {
  test(`${access} Preview guard survives stale DOM invocation and returns recovery to owner B`, async ({ page }) => {
    ...
    try {
      ...
      // Put focus inside the soon-to-be-hidden recovery subtree, then invoke the
      // Preview transition without moving focus through a physical click.
      const restore = page.getByRole('button', { name: 'Restore view', exact: true });
      await expect(restore).toBeVisible();
      await restore.focus();
      await expect(restore).toBeFocused();
      await page.getByRole('button', { name: 'Open export copy', exact: true }).evaluate((button) => {
