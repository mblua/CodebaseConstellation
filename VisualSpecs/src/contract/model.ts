...
export interface GraphModel {
  ...
  readonly nodes: readonly VisualSpecsNode[];
  /** Canonical order: by edge id. */
  readonly edges: readonly VisualSpecsEdge[];
  readonly nodeById: ReadonlyMap<NodeId, VisualSpecsNode>;
  readonly edgeById: ReadonlyMap<EdgeId, VisualSpecsEdge>;
  ...
  readonly source: VisualSpecsSource | undefined;
  readonly generator: VisualSpecsGenerator | undefined;
  readonly stats: Readonly<Record<string, unknown>> | undefined;
}

function compareNodes(a: VisualSpecsNode, b: VisualSpecsNode): number {
...
export function buildModel(doc: VisualSpecsDoc): GraphModel {
  const nodes = [...doc.nodes].sort(compareNodes);
  const edges = [...doc.edges].sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));

  const nodeById = new Map<NodeId, VisualSpecsNode>();
  for (const n of nodes) nodeById.set(n.id, n);
  const edgeById = new Map<EdgeId, VisualSpecsEdge>();
